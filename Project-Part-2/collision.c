...
void gen_random (char *s, const int len)
{
	...
	char charposible[] = "0123456789!\"#$%&'()*+-/. ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
	int lenTotal = sizeof(charposible) -1;
	/* Generate random string using the characters of length 'len'*/
	for (int n = 0; n < len; n++)
	{
		int random = rand() % lenTotal;
		s[n] = charposible[random];
	}
}
...
int main(int argc, char *argv[])
{
	...
	for(j = 0; j < 10000; j++)
	{
		...
		if(match == 1) //check the match found.
		{
			...
			printf ("Initial String: %s     Random String: %s \n", mess, rand_str);
		...
		if(match >= 0)
		{
			...
			printf ("\nRepository String: %s    Random String: %s \n", repo_msg[match], rand_str);
